...
class Robo:
    ...
    def erro_linha(self):
        ...
        return self.vision.center_error
    ...
    def _corrigir_vel(self, vel=setup.VEL_MED):
        ...
        correcao = self._pid_encoder.atualizar(erro)
    ...
    def _corrigir_vel_linha(self, vel=setup.VEL_MED):
        ...
        baseL, baseR = self._corrigir_vel(vel)

        erro = (self.vision.center_error * setup.PESO_ERRO_POS + self.vision.heading * setup.PESO_LOOKAHEAD)

        correcao = self._pid_linha.atualizar(erro)

        velL = baseL - correcao
        velR = baseR + correcao

        velL = max(0,min(1,velL))
        velR = max(0,min(1,velR))
