...
class Desviar(Estado):
    ...
    def executar(self):
        ...
        if self.desvio.concluido:
            ...
            self.planner.log("Desvio interrompido; retomando busca de linha")
            self.planner.mudar_estado(SeguirLinha)
...
class Intersecao(Estado):
    ...
    def executar(self):
        ...
        if curva == Curva.ESQUERDA:

            self.robo.girar(-90)

        elif curva == Curva.DIREITA:

            self.robo.girar(90)

        elif curva == Curva.RETORNO:
...
class EntrarResgate(Estado):
    ...
    def executar(self):
        ...
        if self.fase == "avancar_para_dentro":
            self.robo.andar(setup.AVANCO_ENTRADA_RESGATE_MM, setup.VEL_RESGATE)
